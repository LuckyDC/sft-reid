...
def get_train_iterator_v1(root, p_size, k_size, image_size, random_mirror=False, random_crop=False, random_erase=False,
                          num_worker=4, seed=None):
    transform = []
    ...
    transform.append(augmentor.ToTensor())
    transform.append(augmentor.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))

    if random_erase:
        transform.append(augmentor.RandomErase(mean=[0, 0, 0]))

    transform = augmentor.Compose(transform)

    train = TrainIterator_v1(data_dir=root, p_size=p_size, k_size=k_size,
                             transform=transform, num_worker=num_worker, image_size=image_size, random_seed=seed)
...
def get_test_iterator_v1(dataset, root, batch_size, image_size, num_worker=4):
    transform = [transforms.Resize(image_size),
                 augmentor.ToTensor(),
                 augmentor.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]

    transform = augmentor.Compose(transform)

    train = EvalIterator_v1(data=root, batch_size=batch_size, image_size=image_size, dataset=dataset,
                            transform=transform, num_worker=num_worker)
...
class TrainIterator_v1(DataIter):
    def __init__(self, data_dir, p_size, k_size, image_size, transform=None, num_worker=4, random_seed=None):
...
class EvalIterator_v1(DataIter):
    def __init__(self, data, batch_size, image_size, dataset=None, transform=None, num_worker=4):
        self.batch_size = batch_size
        self.image_size = tuple(image_size)
        self.transform = transform
